...
export function validateOffice(slug, office) {
  ...
  if (office.manualOnly !== undefined && typeof office.manualOnly !== "boolean") {
    errors.push("manualOnly must be true or false");
  }
  // A manual-only office has no PMS connection, so there is no Kolla scope to
  // require. Every other office must have both halves: connector-id alone does
  // not scope a request to one practice, and a missing consumer-id is the one
  // mistake that could read the wrong office's schedule.
  if (!office.manualOnly) {
  ...
  if (b.accentInk !== undefined && !isHex(b.accentInk)) {
    errors.push("branding.accentInk must be a 6-digit hex color");
  }
